...
const user: User = { id: 1, age: 2, isSingle: true, family: [] }; // ok, т.к. name не обязательное свойства
...
### Индексные сигнатуры
Применяются, когда мы не хотим указывать ключи объекта, но хотим указать тип значений. Например, создадим объект с любыми ключами, но все значения обязательно или строка, или число:
...
never - невозможный тип, тип, который не может существовать, возникает как ошибка операций над типами. Например, при попытке создать тип, одновременно отвечающий требованиям строки и числа. type StringAndNumber = string & number; // never
